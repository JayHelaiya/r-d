	...
	public ResponseEntity<CommonResponse> httpRequest(HttpRequest httpRequest, String requestType) {
		...
		try {
			...
			if (!StringUtils.isEmpty(e.getResponseBodyAsString()) && e.getStatusCode() != HttpStatus.NOT_FOUND) {
				//throw new MobifinX1Exception(ResponseCode.CONNECTION_ERROR.getId(), e.getResponseBodyAsString());
			}
			//throw new MobifinX1Exception(ResponseCode.CONNECTION_ERROR.getId(), ResponseCode.CONNECTION_ERROR.getName());
		} catch (Exception e) {
			System.out.println(e);
			//throw new MobifinX1Exception(ResponseCode.CONNECTION_ERROR.getId(),ResponseCode.CONNECTION_ERROR.getName());
		}
